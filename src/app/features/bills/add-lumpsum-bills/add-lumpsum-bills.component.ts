...
export class AddLumpsumBillsComponent implements OnInit, AfterViewInit {
  ...
  /** Shared print CSS — used by both instance and static HTML builders */
  private static getPrintStyles(): string {
    return `
      <style>
        ...
        .description-print {
          display: inline-block; margin: 0 auto; padding: 8px 14px;
          border: 1px solid #d8d8d8; border-radius: 6px; background: #f5f5f5;
          font-size: 14px; font-weight: 600; line-height: 1.45;
          max-width: 85%; white-space: pre-line; word-break: break-word;
          text-align: center; color: #2c3e50;
        }

        .print-summary-footer {
          margin-top: 30px; border-top: 1px solid #e0e0e0; padding-top: 20px; font-size: 15px;
        }
        .summary {
          display: flex; justify-content: flex-end; align-items: center;
        ...
        .challan-mode .summary { display: none !important; }
  ...
  /** Build the inner page body HTML (shared between instance & static) */
  private static buildPageBody(meta: {
    ...
    const esc = AddLumpsumBillsComponent.esc;
    const fmt = AddLumpsumBillsComponent.fmt;
    const dateStr = AddLumpsumBillsComponent.formatDateDDMMYYYY(meta.billDate);
    ...
    const wordsHtml = meta.showAmountInWords !== false
      ? `<div class="summary words-line"><label>In Words :</label><div>${AddLumpsumBillsComponent.amountInWords(grandTotal)}</div></div>`
      : '';

    return `
      <div class="page ${meta.isChallan ? 'challan-mode' : ''}">
  ...
  amountInWords(num: number): string {
    return AddLumpsumBillsComponent.amountInWords(num);
  }
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      if (this.addressBox?.nativeElement) {
        const el = this.addressBox.nativeElement;
  ...
  async saveBill(): Promise<void> {
    ...
    try {
      const exists = await this.billsService.billExists(this.billNumber).toPromise();

      if (exists) {
        const overwrite = await this.toast.confirm({
          message: `Bill ${this.billNumber} already exists. Overwrite?`,
          type: 'warn',
          okText: 'Overwrite',
          cancelText: 'Cancel',
        });
      ...
      await this.billsService.saveBill(billData).toPromise();
  ...
  private async printDocument(mode: 'invoice' | 'challan'): Promise<void> {
    ...
    try {
      ...
      const html = this.buildPrintHtml(mode);
  ...
  private buildPrintHtml(mode: 'invoice' | 'challan'): string {
    const isChallan = mode === 'challan';
    const styles = AddLumpsumBillsComponent.getPrintStyles();
    const body = AddLumpsumBillsComponent.buildPageBody({
      isChallan,
      clientName: this.clientName || '',
      address: this.address || '',
      billNumber: this.billNumber || '',
...
<head>
  <meta charset="utf-8"/>
  <title>${isChallan ? 'Challan' : 'Lumpsum Invoice'} ${AddLumpsumBillsComponent.esc(this.billNumber)}</title>
  ${styles}
</head>
<body>
  ${body}
  ...
  emailBill(): void {
    ...
    const pdfHtml = this.buildPrintHtml('invoice');
  ...
  static buildLumpsumHtml(payload: {
    ...
    return `<!doctype html><html><head><meta charset="utf-8"/>
      <title>Lumpsum Invoice ${AddLumpsumBillsComponent.esc(payload.billNumber)}</title>${styles}
